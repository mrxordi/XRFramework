...
CFileCache::CFileCache(CacheStrategy *pCache, bool bDeleteCache) : CThread("FileCacheStrategy")
{
	m_pCache = pCache;
...
void CFileCache::SetCacheStrategy(CacheStrategy *pCache, bool bDeleteCache)
{
	if (m_bDeleteCache && m_pCache)
		delete m_pCache;
...
ssize_t CFileCache::Read(void* lpBuf, int64_t uiBufSize)
{
	XR::CSingleLock lock(m_sync);
	if (!m_pCache)
	{
