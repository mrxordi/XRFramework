...
bool CWin32File::Open(const CUrl& url)
{
	...
	if (m_hFile != INVALID_HANDLE_VALUE)
	{
		LOGERR("Attempt to open file without closing opened file object first");
		return false;
	}

	std::wstring pathnameW(StringConverter::ConvertPathToWin32Form(url));
...
bool CWin32File::OpenForWrite(const CUrl& url, bool bOverWrite /*= false*/)
{
	...
	{ // newly created file
			...
			&& pathnameW[lastSlash + 1] == L'.')
		{
			...
			if (!hiddenSet)
				LOGWARN("Can't set hidden attribute for file.");
		}
	}
...
ssize_t CWin32File::Read(void* lpBuf, int64_t uiBufSize)
{
	if (m_hFile == INVALID_HANDLE_VALUE)
		return -1;
...
ssize_t CWin32File::Write(const void* lpBuf, int64_t uiBufSize)
{
	if (m_hFile == INVALID_HANDLE_VALUE)
		return -1;
	...
	if (!m_allowWrite)
	{
		LOGERR("Attempt to write file opened for reading");
		return -1;
	}
...
void CWin32File::Flush()
{
	...
	if (!m_allowWrite)
	{
		LOGERR("Attempt to flush file opened for reading");
		return;
	}
...
bool CWin32File::Rename(const CUrl& urlCurrentName, const CUrl& urlNewName)
{
	...
	std::wstring curNameW(StringConverter::ConvertPathToWin32Form(urlCurrentName));
	if (curNameW.empty())
		return false;

	std::wstring newNameW(StringConverter::ConvertPathToWin32Form(urlNewName));
	if (newNameW.empty())
		return false;
...
int CWin32File::Stat(const CUrl& url, struct __stat64* statData)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		{
			statData->st_mtime = WIN32Utils::fileTimeToTimeT(basicInfo.ChangeTime); // most accurate value
			if (statData->st_mtime == 0)
				statData->st_mtime = WIN32Utils::fileTimeToTimeT(basicInfo.LastWriteTime); // less accurate value
			statData->st_atime = WIN32Utils::fileTimeToTimeT(basicInfo.LastAccessTime);
			statData->st_ctime = WIN32Utils::fileTimeToTimeT(basicInfo.CreationTime);
		}
		CloseHandle(hFile);
	}
	if (statData->st_mtime == 0)
		statData->st_mtime = WIN32Utils::fileTimeToTimeT(findData.ftLastWriteTime);

	if (statData->st_atime == 0)
		statData->st_atime = WIN32Utils::fileTimeToTimeT(findData.ftLastAccessTime);
	if (statData->st_atime == 0)
		statData->st_atime = statData->st_mtime;

	if (statData->st_ctime == 0)
		statData->st_ctime = WIN32Utils::fileTimeToTimeT(findData.ftCreationTime);
	if (statData->st_ctime == 0)
